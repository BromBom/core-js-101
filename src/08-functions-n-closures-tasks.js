...
function logger(func, logFunc) {
  function startLog(arr) {
    const substr = JSON.stringify(arr).slice(1, -1);
    logFunc(`${func.name}(${substr}) starts`);
  }
  function endLog(arr) {
    const substr = JSON.stringify(arr).slice(1, -1);
    logFunc(`${func.name}(${substr}) ends`);
  }
...
function partialUsingArguments(fn, ...args1) {
  return (...lastArgs) => fn(...args1, ...lastArgs);
}
...
function getIdGeneratorFunction(startFrom) {
  function* gen() {
    ...
    while (index < 10000) yield index += 1;
  }

  const generation = gen();
  return () => generation.next().value;
}
