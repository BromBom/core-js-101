...
function isInsideCircle(circle, point) {
  if (((Math.abs(point.x - circle.center.x) ** 2)
    + (Math.abs(point.y - circle.center.y) ** 2)) < circle.radius ** 2) return true;
  return false;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (Object.values(obj).includes(currentSymbol)) {
      if (Object.keys(obj).includes(currentSymbol)) {
      ...
      const topElements = stack[stack.length - 1];

      if (obj[currentSymbol] === topElements) {
        stack.pop();
      } else {
        return false;
      }
...
function getCommonDirectoryPath(pathes) {
  let k = pathes[0].split('/').length;
  for (let i = 1; i < pathes.length; i += 1) {
    if (pathes[i].split('/')[0] !== pathes[0].split('/')[0]) return '';
    k = Math.min(k, pathes[i].split('/').length);
    for (let j = 0; j < k; j += 1) {
      if (pathes[i].split('/')[j] !== pathes[0].split('/')[j]) {
        k = j;
        break;
      }
    }
  }
  return `${pathes[0].split('/').slice(0, k).join('/')}/`;
}
...
function getMatrixProduct(m1, m2) {
  const m3 = [...Array(m1.length)].map(() => [...Array(m2[0].length)].map(() => 0));
